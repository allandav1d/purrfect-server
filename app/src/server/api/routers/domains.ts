    ...
    .mutation(async ({ ctx, input }) => {
      ...
      try {
        const serverSettings = await ctx.db.query.serverSettings.findMany({
          where: (settings, { or }) => or(
            eq(settings.key, "server_ipv4"),
            eq(settings.key, "server_ipv6")
          ),
        });

        const serverIpv4 = serverSettings.find(s => s.key === "server_ipv4")?.value || 'localhost';
        const serverIpv6 = serverSettings.find(s => s.key === "server_ipv6")?.value || 'localhost';

        // Verificar IPv4
        let ipv4Status = "failed";
        try {
          const ipv4Addresses = await resolve4(domain.domain);
          ipv4Status = ipv4Addresses.includes(serverIpv4 || "") ? "active" : "failed";
        } catch (error) {
        ...
        if (serverIpv6) {
          try {
            ...
            ipv6Status = ipv6Addresses.includes(serverIpv6) ? "active" : "failed";
          } catch (error) {
        ...
        const status = ipv4Status === "active" || ipv6Status === "active" 
          ? "active" 
          : "failed";
